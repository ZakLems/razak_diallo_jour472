...
**Answer**
If we didn't make a new variable and instead assigned the modified data to the original variable, we would lose that original data. Some aspect of the data could be important later so it's best to keep it easily accesible.
...
**Answer: The column type change from <chr> to <date> and the format is y-m-d**
...
**Answer**
All of the values in the `month` column are marked as the first of each month.
...
**Answer**
The code takes teh data and creates a new column that is the sum of all casted ballots in a county (calculated by adding up each possible candidate and write in votes). Then another new column is create that gets the percentage of votes for Wes Moore by dividing the amount of Moore votes in the county by the new total votes column.
...
**Answer**
The data is now sorted by the percentage each county voted for Moore, with counties that had a higher percentage of Moore votes being at the top.
...
**Answer**
There's not an overwhelming amount of counties in Maryland so it's not super hard to see the data no matter which direction it's sorted. The percentage increase between Garrett and Allegany is interesting but not particular noteworthy. Additionally a similar jump can be seen between Baltimore City and Montgomery County or Charles County and Baltimore County.
...
**Answer**
(Info from the Google gemini overview)
Two similar functions are str_to_lower which makes teh string lowercase and str_to_title() which makes the string title case.
...
**Answer**
The mutate statement first checks if the address field includes Maryland. If so it outputs MD into the new column. It does the same name -> abbreviation dance for California, Washington, Lousiana, Florida, North Carolina, Massachusetts, West Virginia and Virginia. After cycling through all of them, if the address doesn't include any of the listed states, it marks the column as NA.
...
**Answer**
The code groups the expenditures by state and gets the sum of all the expenditures for that state. The answer could benefit from a few commas to make it more readable for me but otherwise it makes sense.
...
1.  I can use the filter function to get a professor to seat ratio per department for only the fall and spring terms. 

2.  What day of the week do most expenditures happen?

3.  You can filter out courses with 0 sections to get a better look at headline numbers like largest seat count or biggest department.
