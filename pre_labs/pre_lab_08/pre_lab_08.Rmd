...
The code takes the 911 overdose data from Baltimore County and creates a new column titled month. This column takes the month of the overdose call (with Label=TRUE indicating the month should be in abbreviated format). The data is then grouped by the new month column and the total calls for each month are tallied in the total_calls column. The overdose calls for each month are then sorted from greatest to least.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
A graph from January to December counting the total call values from our data. All the bars are grey and it's hard to tell an exact number from it.
```{r}
baltco_911_calls_by_month |>
  ggplot() +
...
The months are now ordered from lowest total calls to most.
```{r}
baltco_911_calls_by_month |>
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
The graph's y-axis is now the months and the x-axis is the count.
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
The grid is transparent instead of grey.
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
The background is blue and the horizontal lines of the grid are gone. There's also more seperation between the x values and the chart. The month labels are kind of crashing into the y-axis label.
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
The graph has a title and cites the source of the data. The labels better match what the graph is trying to represent instead of directly outputting what's in the code/dataframe.
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
The call data is being grouped by individual dates and another column shows the total calls for each date.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
I see a black line running along a chart indicating the total calls for each day. The x-axis is labeled by the date which has been simplified to a month-year format.
```{r}
baltco_911_calls_by_date |>
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
A slightly better look at the dates for each week in the data set.
```{r}
baltco_911_calls_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
