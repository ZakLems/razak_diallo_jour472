...
The dataframe shows Maryland Board of Physicians Disciplinary reports with each doctor, their first and last name, the type of alert, the date of the alert, and text documentation of their alert.
```{r}
...
The unique function removes repeats of "dog" and "cat" from the list. It keeps "Dog" and "dog" because their capitalization is different. 
```{r}
...
The function outputs a dataframe with a list of unique words from every text cell. The list doesn't factor in repeats across rows.
```{r}
...
Not super interesting. "the" is obviously the top word because it's one of the most common words in the language. The most interesting word in the list to me is her at spot 25. But if you add just one more spot, you see that he is right after.
...
It's a bit more interesting. I think consent being so high could be something to look at. 2017 is the highest year but we could've gotten that by doing analysis on the date column.
```{r}
unique_words |>
...
**Task** Run the following code and describe the results. **Answer**
The result is a list of two word combinations. The most interesting to me are unprofessional conduct and medical records.
```{r}
alerts |>
...
Words like cancer seem more neutral to me in this context.
```{r}
