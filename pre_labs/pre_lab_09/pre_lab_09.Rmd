...
https://datawrapper.dwcdn.net/w7HDo/1/
...
md_gov_county_perct <- md_gov_county |> 
  mutate(
    total_votes = rowSums(select(md_gov_county, 3:8)),
    moore_perct = (Moore/total_votes)*100
  )

write_csv(md_gov_county_perct, "data/md_gov_county_1.csv")
```
...
It's zoomed in and at an angle on Maryland instead of the entire United States
...
When you hover over a county it shows its name and the percentage of the population that voted for Wes Moore.
...
https://datawrapper.dwcdn.net/U0C9u/1/
