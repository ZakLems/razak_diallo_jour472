...
ObjectID, ZIPCODE2, ZIPNAME, EXISTING, ZIPCODE1, SHAPE_Length, SPAHE_Area, geometry
...
There are zipcodes, names of zipcodes (which seem to be cities), length and area of shapes, and polygon coordinates.
```{r}
View(md_zips)
```
...
The state of Maryland divided by zipcode. There is also a longitude and latitude grid.
```{r}
md_zips |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output seems to be all the counties in the country.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The 24 counties that make up our beautiful state
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Map of Maryland subdivided by county
```{r}
md_counties |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The boundaries between zipcodes are a bit more pronounced and the bay has been completely removed with county borders.
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
It has zipcodes and the amount of foreclosures in March of 2023
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The zipcodes and geometric data have been matched with the foreclosure data
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The zipcodes now have a gradient from dark to light blue representing the amount of foreclosures. There's a lot in Prince George's County.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
One database with teh population of each maryland county and another with the amount of foreclosure notices
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The county population now has the geometry of each county 
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              year = 2021,
...
**Task** Run the following code. Describe the output in the space below. **Answer**
md_county_population is a different format than md_county_notices when it comes to county names
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The ", maryland" of the population database is now removed.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The population is now paired with the notices for foreclosure
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The per capita foreclosure rate is now there
```{r}
md_pop_with_foreclosures <- md_county_population |>
  left_join(md_county_notices, join_by(county)) |> 
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Charles County has the highest foreclosure rate and is the lightest-bluest on the map
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The color scale and differences are more abrupt
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
...
North central Maryland has a relatively low foreclosure rate while the South is getting up there. Urban centers are also not doing so hot (the Baltimore city sentence).
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
