...
```{r echo=FALSE}
#census_api_key("erasing cause github is public", install=TRUE)
```
...
Nevada has the largest percentage growth at a 35% increase between 2000 and 2010.
```{r}
...
The Maryland county with the highest median household income is Howard County.
```{r}
...
Estimated median household income for Maryland counties
```{r}
md |>
...
Yes, no margin of era overlaps with Howard
...
Baltimore City saw the largest increase in median income. No county had their median income fall between 2010 and 2021.
```{r}
md10 |>
...
What relationship does the percentage of high school graduates that went on to enroll in college have to the median income of their zip code. The answer may be difficult to interpret because some high schools take students outside of their zip code.
```{r}
...
High schools that have 60% or more graduates immediately enrolling into college have a higher median income than high schools with less than 40% of graduates immediately enrolling into college.
```{r}
...
What local school system has the highest average median income of zip codes with high schools in them. 