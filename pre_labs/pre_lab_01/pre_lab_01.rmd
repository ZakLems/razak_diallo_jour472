...
**Answer**
79366 observations and 9 variables
...
The id column type is chr and the seats column is dbl. I'm assuming that's equivalent to a string and a number respectively.
...
**Answer**
The first two lines takes the umd_courses data and groups them by terms. Next the summarise function counts the amount of classes in each term and shows it as a new row.
...
Unlike the previous codeblock, this one sorts the terms by the count of classes column, with the term with the most classes at the top.
...
**Answer**
This codeblock groups the observations by term and department. It then sorts it by term and then alphabetically based on the department.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer**
This code block counts the total amount of available seats offered each term and then sorts the data by that amount. The term with the most seats being first.
...
This codeblock shows more columns (count_classes, mean_seats and median_seats). Generally the top terms are arranged in a way that coincides with the order if the data was arranged by class count. 
...
**Answer**
The data is grouped by department so each entry is a collection of all classes taught in the department across all terms. The addition of minimum and maximum seats allows us to see the smallest and largest class sizes the departments have ever offered, although some courses offer 0 seats which muddies any analysis a bit. The data is sorted by the total amount of seats the department has offered.
...
1.  What department has the smallest instructor to available seats ratio?

2.  What instructor has taught the most total seats? 

3.  What term accounted for the greatest increase in unique courses offered?
