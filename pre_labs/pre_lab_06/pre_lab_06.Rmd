...
They seemcolumn structure is the same except for 2024 which is missing the Green Party. The order of the columns and some capitalization varies across the datasets.
...
Because there are 24 counties in the population database, there should be 24 rows. There are 24 rows.
```{r}
...
It tells us what the join_by method was, in this case by county.
```{r}
...
Prince George's County has the highest Black 18+ population and 78% of the voters in the county are democrats.
```{r}
...
Howard county has the highest Asian 18+ population with 52% of voters being democrats. Howard and Montgomery county are outliers among the other counties, the only two with an 18+ Asian population over 10%.
```{r}
maryland_population_with_voters_and_demographics |>
