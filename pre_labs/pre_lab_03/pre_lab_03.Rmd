...
The first line says "one or more parsing issues, call `problems()` on your data frame for details." When you call problems() it shows a data frame that lists rows in the payments database that have a conflict. Most of them seem to be expecting a double (number) in column 4 (zipcode) but instead are getting mixtures of letters and numbers.
...
There aren't any parsing errors when I run the code. The reader library has enough data to properly assign columns to chr or dbl.
...
"Vendor Name" is chr, "Amount" is dbl and "Date" is chr.

```{r}
payments |> mutate(Date=mdy_hms(Date))
glimpse(payments)
...
553 zip codes do not have the are less than five characters. These records could be typos, considering how large the data set is, 553 feels like a realistic amount of mistakes.
...
There are some uncategorized recipients and 50 recipients categorized as just "L."
...
There are three payments below $300 dollars and then the rest are relatively close to each other, steadily increasing into the thousands.
...
The headers are lowercase and any spaces are replaced with underscores (to my chagrin as a camelCase enthusiast).
...
1. Creates a variable for the cleaned dataframe by acting on the original dataframe
2. runs clean_names() which standardizes the columns to lowercase and underscore seperation
3. renames the grantor column to source
4. modifies the source, grantee and description columns so all text is uppercase.
...
There is potentially 58 duplicates in the dataset.
...
The new dataframe has 19453 rows.
...
The zip5 column gets rids of every character after the 5th one, meaning there are no hyphenated zip codes.
...
I'm not sure if you meant just the "MARYLAND TOURISM BOARD" or also when they're included along with a the department of commerce and also if there's a difference between the "tourism development board" and the "tourism board" so I just did a string detect for tourism and board which gave 8 sources.

The zip code that has gotten the most money is 21202 which is in Baltimore City and covers part of the inner harbor. It makes sense that it's the top recipient because it's in the most populous city and the urban core of Maryland. Moreover, the National Aquarium is there, a major tourist attraction for the state.

```{r}
cleaned_md_grants_loans |> 
  filter(str_detect(source, "TOURISM") & str_detect(source, "BOARD")) |> 
  group_by(zip5) |> 
  summarize(
